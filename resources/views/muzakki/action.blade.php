...
<a class="btn btn-sm btn-icon btn-secondary" data-bs-toggle="tooltip" title="Edit Mustahik" href="{{ route('editmuzzaki', $code) }}">
...
->
    ...
    ?>
    <a class="btn btn-sm btn-icon btn-danger" onclick="return confirm('{{$message}}') ? document.getElementById('kategori-delete-{{$code}}').submit() : false" data-bs-toggle="tooltip" title="Delete muzakki" href="#">
        <span class="btn-inner">
    ...
    <form action="{{route('muzakki.destroy',$code)}}" id="kategori-delete-{{$code}}" method="post">
        @method('delete')
        @csrf()
    </form>
</div>